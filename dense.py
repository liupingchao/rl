...
def dataprocess(stk,changeratio,pb,pettm,pvv,fr5):
	...
	try:
		...
		for i in range(trainlen-1,len(tmpdata)):
			...
			temp = temp.reshape(1,trainlen)
			temp2 = np.array(returntrans(frdata.ix[i]))
			temp2 = temp2.reshape(1,2)
...
if __name__ == "__main__":
	...
	for j in samplelist[0:10]:
		...
		##sigle thread
		# tempresultx,tempresulty=dataprocess(stk,changeratio,pb,pettm,pvv)
		# if j==samplelist[0]:
		# 	trainx = tempresultx
		# 	trainy = tempresulty
		# else:
		# 	trainx= np.concatenate((trainx,tempresultx),axis = 0)
		# 	trainy = np.concatenate((trainy,tempresulty),axis = 0)
	...
	model = Sequential()
	model.add(Dense(40, input_shape=(timesteps, )))  # returns a sequence of vectors of dimension 100
